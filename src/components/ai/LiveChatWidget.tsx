...
export function LiveChatWidget() {
  ...
  const refreshAgentStatus = useCallback(() => {
    chatApi.getAgentStatus()
      .then((res) => setAgentsOnline(res.agents_online))
      .catch(() => setAgentsOnline(true));
  }, []);

  // Check if any human agent is online when widget opens
  useEffect(() => {
    if (!open) return;
    refreshAgentStatus();
    const interval = setInterval(refreshAgentStatus, 45000);
    return () => clearInterval(interval);
  }, [open, refreshAgentStatus]);
  ...
  const noAgentsOnline = agentsOnline === false;
    ...
    return (
      <div className="flex-1 flex flex-col bg-gray-50 h-full overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-500">
                {isClosed ? "Chat ended" :
                 isWaiting ? (noAgentsOnline ? "No agents online right now" : "Finding an available agent…") :
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {isWaiting && noAgentsOnline ? renderOfflineAgentNotice(true) : null}
